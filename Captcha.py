...
def preprocess_image(image_path, img_width=100, img_height=40):
    ...
    if img is None:
        raise ValueError(f"Failed to load image. Check the file format and path: {image_path}")
...
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
# 5. Train the model
model.fit(X, y_encoded, epochs=5, batch_size=32, validation_split=0.2)
# 6. Decode new CAPTCHA
def decode_captcha(model, image_path, char_list, char_to_index):
    img = preprocess_image(image_path)
    img = np.expand_dims(img, axis=0)
...
captcha_image_path = 'captchas/8VV3U.png'  # Adjust this path if needed
